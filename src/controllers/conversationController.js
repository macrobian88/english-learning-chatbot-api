...
async function listConversations(req, res, next) {
  try {
    ...
    const topics = await Topic.find({ topic_id: { $in: topicIds } }).select('topic_id title').lean();
    const topicMap = topics.reduce((acc, t) => { acc[t.topic_id] = t.title; return acc; }, {});

    const result = conversations.map(conv => ({
      topic_id: conv.topic_id,
      title: topicMap[conv.topic_id] || 'Unknown Topic',
      message_count: conv.messages.length,
      last_message: conv.messages.length > 0 ? conv.messages[conv.messages.length - 1].content.substring(0, 100) : null,
