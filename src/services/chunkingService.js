...
async function processTopicFiles(topicId, title, files, metadata = {}) {
  ...
  if (existingTopic) throw new Error(`Topic ${topicId} already exists`);
  ...
  for (let i = 0; i < allChunks.length; i += BATCH_SIZE) {
    ...
    logger.debug(`Inserted chunk batch ${Math.floor(i / BATCH_SIZE) + 1}`);
  }
...
async function updateTopicFiles(topicId, title, files, metadata) {
  const existingTopic = await Topic.findOne({ topic_id: topicId });
  if (!existingTopic) throw new Error(`Topic ${topicId} not found`);

  await Chunk.deleteMany({ topic_id: topicId });
  await Topic.deleteOne({ topic_id: topicId });

  return processTopicFiles(topicId, title || existingTopic.title, files, metadata || existingTopic.metadata);
}
...
async function deleteTopic(topicId) {
  ...
  logger.info(`Deleted topic ${topicId} and all associated chunks`);
  return { deleted: true, topic_id: topicId };
}

module.exports = { splitIntoChunks, processTopicFiles, updateTopicFiles, deleteTopic };
